...
from flex_evals.schemas import (
    TestCase, Output, Check, CheckResult, TestCaseResult, EvaluationRunResult,
    ExperimentMetadata
)
...
class TestExampleCheck(BaseCheck):
    ...
    def __call__(self, arguments: Dict[str, Any], context: EvaluationContext) -> Dict[str, Any]:
        expected = arguments.get("expected", "Paris")
        actual = arguments.get("actual", context.output.value)
