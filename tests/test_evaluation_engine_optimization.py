...
class TestFlattenUnflatten:
    ...
    def test_flatten_checks_basic(self):
        ...
        # Check tracking
        assert tracking[0] == (0, 0, False, 0)  # test_idx=0, check_idx=0, is_async=False, flattened_idx=0
        assert tracking[1] == (1, 0, False, 1)  # test_idx=1, check_idx=0, is_async=False, flattened_idx=1
    ...
    def test_unflatten_preserves_check_order(self):
        ...
        result1 = CheckResult(check_type="check1", status="completed", results={}, resolved_arguments={}, evaluated_at=datetime.now(UTC))
        result2 = CheckResult(check_type="check2", status="completed", results={}, resolved_arguments={}, evaluated_at=datetime.now(UTC))
        result3 = CheckResult(check_type="check3", status="completed", results={}, resolved_arguments={}, evaluated_at=datetime.now(UTC))
    ...
    def test_flatten_unflatten_complex_scenario(self):
        ...
        assert tc2_tracking[0][2] == False  # sync
        assert tc2_tracking[1][2] == False  # sync
        assert tc2_tracking[2][2] == True   # async
        assert tc2_tracking[3][2] == False  # sync
...
class TestPerformanceOptimization:
    ...
    def test_sync_checks_have_no_async_overhead(self):
        ...
        assert total_time < 1  # generous upper bound
