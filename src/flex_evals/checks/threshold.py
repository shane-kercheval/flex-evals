...
class ThresholdCheck(BaseCheck):
    ...
    @classmethod
    def validate_threshold_values(cls, v: float | int | JSONPath | None) -> float | int | JSONPath | None:
    ...
    def __call__(self) -> dict[str, Any]:
        ...
        if isinstance(self.min_inclusive, JSONPath):
            raise RuntimeError(f"JSONPath not resolved for 'min_inclusive' field: {self.min_inclusive}")
        if isinstance(self.max_inclusive, JSONPath):
            raise RuntimeError(f"JSONPath not resolved for 'max_inclusive' field: {self.max_inclusive}")
